    ...
    public function __construct()
{
    ...
    public function show($id)
    {
        ...
        {
            if (Item::find($id)->imageEntry()->where('record_id', $id)->where('record_type', 0)->get())
            {
                $imageview=Item::find($id)->imageEntry()->where('record_id', $id)
                                                        ->where('record_type', 0)->get();
    ...
    'rights', 'relation', 'publisher', 
     ...
     'contributor');
        ...
        $files=$files->move(base_path() . '/storage/app/img/', $filename);
        ...
        if ($request->output)
        {
            $imagestoCheckout=Item::find($id)->imageEntry()->where('record_id', $item->id)
            												->where('record_type', 0)
            												->get();
        ...
        if ($request->delete)
        {
           ...
           $imagedelete=Image::where('filename', $request->delete)
                                ->where('record_id', $item->id)
                                ->where('record_type', 0)->first();
            
            Dublin::destroy($imagedelete->id);
            Image::destroy($imagedelete->id);
